...
import (
	...
	"github.com/IBM/integrity-enforcer/operator/pkg/apis"
	"github.com/IBM/integrity-enforcer/operator/pkg/controller"
	"github.com/IBM/integrity-enforcer/operator/version"
...
func main() {
	...
	err = leader.Become(ctx, "integrity-enforcer-operator-lock")
	if err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	if err := iespol.AddToScheme(mgr.GetScheme()); err != nil {
		log.Error(err, "")
		os.Exit(1)
	}

	if err := iepol.AddToScheme(mgr.GetScheme()); err != nil {
		log.Error(err, "")
		os.Exit(1)
	}

	if err := ec.AddToScheme(mgr.GetScheme()); err != nil {
		log.Error(err, "")
		os.Exit(1)
	}

	if err := iedpol.AddToScheme(mgr.GetScheme()); err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	if err := scc.AddToScheme(mgr.GetScheme()); err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
