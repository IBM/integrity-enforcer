...
func BuildDeploymentForCR(cr *apiv1alpha1.IntegrityVerifier) *appsv1.Deployment {
	...
	volumemounts = []v1.VolumeMount{
		{
			MountPath: "/iv-app/public",
			Name:      "log-volume",
		},
	}

	volumes = []v1.Volume{
		SecretVolume("iv-tls-certs", cr.GetWebhookServerTlsSecretName()),
	...
	serverContainer := v1.Container{
		...
		Env: []v1.EnvVar{
			...
			{
				Name:  "VERIFIER_CONFIG_NAME",
				Value: cr.GetVerifierConfigCRName(),
			},
			{
				Name:  "CHART_BASE_URL",
			...
			{
				Name:  "VERIFIER_CM_RELOAD_SEC",
				Value: strconv.Itoa(int(cr.Spec.Server.VerifierCmReloadSec)),
			},
			{
				Name:  "VERIFIER_POLICY_RELOAD_SEC",
	...
	loggerContainer := v1.Container{
		...
		Env: []v1.EnvVar{
			...
			},
			{
				...
				Value: "/iv-app/public/events.txt",
			},
		},
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      cr.GetIVServerDeploymentName(),
...
// iv-rule-table-lock ConfigMap
func BuildRuleTableLockConfigMapForCR(cr *apiv1alpha1.IntegrityVerifier) *v1.ConfigMap {
	labels := cr.Spec.MetaLabels
...
// iv-ignore-table-lock ConfigMap
func BuildIgnoreRuleTableLockConfigMapForCR(cr *apiv1alpha1.IntegrityVerifier) *v1.ConfigMap {
	labels := cr.Spec.MetaLabels
...
// iv-force-check-table-lock ConfigMap
func BuildForceCheckRuleTableLockConfigMapForCR(cr *apiv1alpha1.IntegrityVerifier) *v1.ConfigMap {
	labels := cr.Spec.MetaLabels
