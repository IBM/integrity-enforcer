...
- Integrity Verifier admission controller is installed in a dedicated namespace (e.g. `integrity-verifier-operator-system` in this document). It can be installed by operator. (see [Integrity Verifier Custom Resource](README_IV_OPERATOR_CR.md) for detail install options.)
​