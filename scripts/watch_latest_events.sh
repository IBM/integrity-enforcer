...
kubectl logs -f --tail 20 -n ${IV_NS} ${IV_SERVER_POD} -c forwarder | grep --line-buffered ' fw.events: ' | awk '{sub(/^.* fw.events: /, "", $0);print $0;fflush()}' | jq --unbuffered -r '. | [.namespace, .allowed, .kind, .name, .operation, .userName, ."sig.signer.displayName", .msg] | @tsv' 
