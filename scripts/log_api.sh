...
ISHIELD_SERVER_POD=`kubectl get pod -n ${ISHIELD_OP_NS} | grep integrity-shield-api | grep Running | awk '{print $1}'`
if [ -z "$ISHIELD_SERVER_POD" ]; then
    echo "ISHIELD_SERVER_POD is empty. There is no running integrity-shield-api"
    exit 1
fi

kubectl logs -f -n ${ISHIELD_OP_NS} ${ISHIELD_SERVER_POD} 