...
func (self Parameters) ProtectMatch(reqFields map[string]string) (bool, *common.Rule) {
	// parameters check protection only when AdditionalProtectRules are defnied
	if len(self.AdditionalProtectRules) == 0 {
		return true, nil
	}
	...
	for _, rule := range self.AdditionalProtectRules {
		if strictMatch && rule.StrictMatchWithRequest(reqFields) {
			return true, rule
		} else if !strictMatch && rule.MatchWithRequest(reqFields) {
			return true, rule
