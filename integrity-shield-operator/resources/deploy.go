...
//deployment
func BuildAPIDeploymentForIShield(cr *apiv1alpha1.IntegrityShield) *appsv1.Deployment {

	var servervolumemounts []v1.VolumeMount
	var volumes []v1.Volume

	volumes = []v1.Volume{
		SecretVolume("ishield-api-certs", cr.GetAPITlsSecretName()),
		EmptyDirVolume("log-volume"),
		EmptyDirVolume("tmp"),
	}
	for _, keyConf := range cr.Spec.KeyConfig {
	...
	apiContainer := v1.Container{
		...
		Env: []v1.EnvVar{
			...
			{
				Name:  "SHIELD_CM_RELOAD_SEC",
				Value: strconv.Itoa(int(cr.Spec.API.ShieldCmReloadSec)),
			},
			{
				Name:  "SHIELD_POLICY_RELOAD_SEC",
				Value: strconv.Itoa(int(cr.Spec.API.EnforcePolicyReloadSec)),
			},
		},
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      cr.GetIShieldAPIDeploymentName(),
			Namespace: cr.Namespace,
