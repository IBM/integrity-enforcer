...
func (self *RuleTable) NamespaceList(verifierNamespace string) []string {
...
func (self *RuleTable) Match(reqFields map[string]string, verifierNS string) (bool, []*v1.ObjectReference) {
	matchedSources := []*v1.ObjectReference{}
	for _, item := range *self {
		if item.Match(reqFields, verifierNS) {
...
func (self *RuleItem) Match(reqFields map[string]string, verifierNS string) bool {
	reqNamespace := ""
	if tmp, ok := reqFields["Namespace"]; ok && tmp != "" {
		reqNamespace = tmp
	}
	// ignore this RuleItem if the namaespace is different
	if !self.CheckNamespace(reqNamespace, verifierNS) {
		return false
	}
...
func (self *RuleItem) CheckNamespace(reqNamespace, verifierNamespace string) bool {
...
func NewRuleTableFromProfile(sProfile rspapi.ResourceSigningProfile, tableType RuleTableType, verifierNamespace string, reqc *common.ReqContext) *RuleTable {
	...
	if source.Namespace == verifierNamespace && sProfile.Spec.TargetNamespaceSelector != nil {
