...
import (
	...
	pgp "github.com/IBM/integrity-enforcer/verifier/pkg/util/sign/pgp"
	x509 "github.com/IBM/integrity-enforcer/verifier/pkg/util/sign/x509"
)
...
func NewVerifier(verifyType VerifyType, signType SignatureType, verifierNamespace string, keyPathList []string) VerifierInterface {
	if signType == SignatureTypeResource || signType == SignatureTypeApplyingResource || signType == SignatureTypePatch {
		return &ResourceVerifier{Namespace: verifierNamespace, VerifyType: verifyType, KeyPathList: keyPathList}
	} else if signType == SignatureTypeHelm {
		return &HelmVerifier{Namespace: verifierNamespace, VerifyType: verifyType, KeyPathList: keyPathList}
	}
	return nil
}
...
func (self *ResourceVerifier) Verify(sig *GeneralSignature, reqc *common.ReqContext, signingProfile rspapi.ResourceSigningProfile) (*SigVerifyResult, error) {
	...
	if sig.option["scopedSignature"] {
		...
		if reqc.IsCreateRequest() {
			...
			// for UPDATE request, IV will confirm that no value is modified except attributes in `scope`.
...
func (self *ResourceVerifier) MatchMessage(message, reqObj []byte, protectAttrs, unprotectAttrs []*profile.AttrsPattern, allowDiffPatterns []*mapnode.DiffPattern, resScope string, signType SignatureType) (bool, string) {
	...
	// because iv-sa does not have role for creating "any" resource at cluster scope
	if resScope == "Cluster" {
